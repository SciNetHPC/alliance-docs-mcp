...
url: "https://docs.alliancecan.ca/wiki/Database_servers/fr"
category: "General"
last_modified: "2025-09-15T18:25:02Z"
page_id: 6856
display_title: "Serveurs de bases de données"
language: "fr"
---
...
REMARQUE : À compter du 13 janvier 2025, la capacité de la grappe Graham sera réduite à 25% (voir   Réduction de la capacité) jusqu'à ce que le nouveau système Nibi soit disponible. Aucun serveur de bases de données ne sera offert pendant la transition.
...
Le serveur MariaDB est l'instance cedar-mysql-vm (nom long, cedar-mysql-vm.int.cedar.computecanada.ca). Si vous possédez un compte sur le serveur MySQL, vous pouvez y accéder uniquement par le nœud de connexion (cedar.computecanada.ca), les nœuds de calcul ou le portail Cedar.
...
Lancez le client mysql pour vous connecter au serveur MySQL. Une version moins récente du client est disponible sans que vous ayez à charger un module, mais vous n'aurez pas les dernières fonctionnalités du serveur. Nous vous recommandons de charger une version plus récente du client avec
...
# Connectez-vous à MySQL avec mysql.
# Lancez la commande USE database;
#* database est le nom de la base de données où se trouve la table que vous voulez partager
# Lancez la commande GRANT priv_type ON mytable TO ′user′@'172.%';
...
Nous créerons un compte PostgreSQL pour lequel le nom d'utilisateur sera celui de votre compte avec l'Alliance. Vous aurez accès à une base de données dont le nom sera ̠db.  Vous ne pouvez pas créer une base de données, mais si vous en avez besoin de plus d'une, écrivez au soutien technique.
...
> -- Give kim connect access to the database
...
> -- Connect to the database containing the table to be accessed
 kim_db> \c david_db
...
> -- Revoke kim's select-only access to a table called mytable
...
Les étapes pour obtenir et utiliser un compte sur le serveur MySQL de Graham sont semblables à celles décrites ci-dessus pour Cedar, sauf qu'il faut remplacer le module avec
...
Pour y accéder, contactez le soutien technique.
