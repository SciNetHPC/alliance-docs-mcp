...
If you switch between different RC files you should be careful of environment variables which may still be set from the previous RC file, these may cause your OpenStack client commands to fail. Either ensure that all environment variables set in the RC file are unset with unset , or start a fresh new session without any of the RC environment variables set.
...
When running in interactive mode a list of available commands can be seen by typing help at the OpenStack prompt. The available commands are categorized by groups; many of the most commonly used groups are listed below. A list of commands belonging to a command group can be obtained by typing help . To get help on a particular command (e.g. options and arguments of the command) one can type help  . Note that many commands are available only to OpenStack administrators and will return an error message if a non-administrator tries using it. For this reason a list of available commands grouped by command groups is provided below to allow one to easily identify commands available to the average user.
...
* nova: for working with servers, see OpenStack docs for nova command.
* glance: for working with images, see OpenStack docs for glance command.
* cinder: for working with volumes, see OpenStack docs for cinder command.
* heat: for working with orchestration, see OpenStack docs for heat command.