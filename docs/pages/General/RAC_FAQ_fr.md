---
title: "RAC FAQ/fr"
url: "https://docs.alliancecan.ca/wiki/RAC_FAQ/fr"
category: "General"
last_modified: "2025-08-29T15:28:46Z"
page_id: 30427
display_title: "FAQ pour le concours d'allocation de ressources"
language: "fr"
---

Si vous avez des questions en rapport avec la terminologie, consultez le glossaire technique.
...
===Pourquoi devons-nous allouer nos ressources?===
...
* Si vous avez entré plusieurs plusieurs rôles actifs de professeure ou professeur dans CCDB, assurez-vous que votre rôle le plus à jour est indiqué comme étant votre rôle principal. Dans  CCDB, allez à la page d'accueil et au besoin, cliquez sur le bouton Sélectionner comme rôle principal correspondant à votre rôle principal.

Le non-respect de l'une de ces conditions pourrait entraîner des problèmes si votre demande est retenue. Si vous avez des questions, écrivez à  allocations@tech.alliancecan.ca.

=== Qu'est-ce que le concours pour l'allocation des ressources? ===
Les demandes soumises au concours font l’objet de deux examens :

* un examen par les pairs effectué bénévolement par plus de 90 pairs experts en provenance de divers établissements d’enseignement canadiens qui jugent du mérite des projets soumis. Chaque demande reçoit une note qui sert à guider objectivement les décisions relatives à l’allocation des ressources; ET
...
Pour plus d’information, voir la page Concours pour l'allocation de ressources.

=== Le concours pour l’allocation des ressources comporte-t-il un volet de financement? ===
...
=== Qu’est-ce le processus de demande accélérée? ===
...
Certaines chercheuses, chercheurs et petits groupes de recherche souhaitent conserver leur allocation actuelle sans modifier l'approche scientifique ou avec des modifications minimes, voire nulles, des ressources nécessaires. Pour réduire la nécessité de réévaluer ces propositions, nous avons mis en place le processus de demande accélérée.
...
Pour plus d’information, voir la page  Guide de préparation des demandes accélérées.
...
Pour plus d'information, voir la page Ressources disponibles par le concours d'allocation de ressources.
...
=== Devrais-je consulter l'équipe technique avant de faire une demande d'allocation de ressources? ===
...
Pour soumettre une demande, vous devez détenir un compte actif dans CCDB (voir la page Demander un compte CCDB). Toutes les demandes doivent être soumises directement dans CCDB.
...
Oui. Pour vous aider à préparer votre demande, nous avons mis à votre disposition une liste des termes employés dans le contexte du concours (voir la page  Terminologie du concours pour l’allocation de ressources).
...
=== Mon équipe de recherche est petite et reçoit peu de financement. Suis-je tout de même admissible au concours pour l’allocation de ressources? ===
Oui. Les chercheuses principales et chercheurs principaux en milieu académique peuvent soumettre une demande, peu importe la taille de l’équipe de recherche et le montant du financement reçu.

=== La date limite de soumission des demandes peut-elle être reportée? ===
...
Les résultats du concours seront annoncés en mars. Vous recevrez un courriel indiquant tous les renseignements nécessaires et les étapes à suivre pour accéder à vos ressources. Une fois que toutes les participantes et participants auront été personnellement informés, nous publierons sur notre site Web la liste des projets retenus, la quantité de ressources allouées à chaque projet et la valeur des allocations (voir Rapports annuels pour le concours d'allocation de ressources).
...
Pour plus d’information, voir la page Allocation et ordonnancement.
...
Avant d’avoir accès aux demandes, les membres des comités d'examen par les pairs doivent signer une entente de confidentialité et de non-divulgation et accepter de se conformer à notre politique en matière de conflit d’intérêts.

=== Puis-je choisir le comité d'examen par les pairs qui étudiera ma demande? ===
Oui. Nous nous réservons toutefois le droit de soumettre celle-ci à un autre comité au besoin.

=== Quel est le rôle du comité d'administration du programme d’accès aux ressources? ===
Ce comité applique les politiques du concours, supervise le déroulement de celui-ci et offre du soutien technique et administratif aux divers comités d’examen par les pairs. Bien qu’il incombe à ces derniers de déterminer le mérite de chaque proposition et d’allouer les ressources appropriées, le comité veille à répartir ces allocations en fonction des ressources disponibles. Ainsi, les présidents des comités d’examen par les pairs doivent parfois cautionner une réduction proportionnelle de l’ensemble des allocations, laquelle se fait en fonction de la note attribuée par le comité scientifique et non selon une évaluation du comité d’administration du programme d'accès aux ressources. Ce dernier est composé de personnes qui ont une connaissance approfondie des ressources et des processus du concours : il est composé de dirigeants techniques et administratifs représentant chaque région (Groupe de l’IRN de la C.-B., IRN des Prairies, Calcul Ontario, Calcul Québec et ACENET).

=== Dois-je préparer une lettre d’intention pour soumettre une demande au concours? ===
Non. Une lettre d’intention n’est pas requise.

== RESSOURCES POUR LES GROUPES DE RECHERCHE ==

=== Qu'est-ce que le processus de demande pour les groupes de recherche? ===
...
=== Si j’ai reçu une allocation pour mon groupe de recherche l’an dernier, dois-je soumettre une demande encore cette année? ===
...
=== Les demandes soumises au concours pour l’allocation de ressources sont-elles toutes acceptées? ===
...
=== Qu'est-ce que le processus de demande pour les plateformes et portails de recherche et en quoi diffère-t-il du processus pour les groupes de recherche? ===
...
Pour plus d’information, voir  Guide de préparation des demandes pour le concours d’allocation de ressources.
...
=== Est-il possible de soumettre plus d'une demande au concours pour l’allocation de ressources aux plateformes et portails de recherche? ===
...
Voir Rapport annuel sur l’avancement des projets de plateformes et portails de recherche.

== ALLOCATIONS ==

=== Que signifie le terme « allocation »?? ===
...
=== Les ressources de calcul qui me sont allouées sont-elles garanties? ===
...
Les chercheuses principales et chercheurs principaux peuvent accéder à de petites quantités de ressources infonuagiques, de calcul et de stockage au moyen du service d’accès rapide. Des ressources de calcul sont aussi offertes pour utilisation opportuniste, sans priorité spéciale.
...
=== Les allocations actuelles ont-elles priorité au moment de décider des prochaines allocations? ===
...
Oui. Pour plus d’information, voir Ressources disponibles par le concours d'allocation de ressources..