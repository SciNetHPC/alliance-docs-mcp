...
url: "https://docs.alliancecan.ca/wiki/Running_jobs/fr"
category: "General"
last_modified: "2026-02-23T11:41:58Z"
page_id: 2540
display_title: "Exécuter des tâches"
language: "fr"
---
...
Si vous n'avez jamais travaillé sur une grappe partagée de grande taille, nous vous recommandons de lire d'abord Qu'est-ce qu'un ordonnanceur?

L'ordonnancement des tâches se fait à l'aide de
Slurm Workload Manager.
La documentation Slurm est préparée par SchedMD. Si vous utilisez PBS/Torque, SGE, LSF ou LoadLeveler, ce tableau de correspondance des commandes vous sera utile.
...
Sur les superordinateurs d'usage général, cette tâche réserve un (1) cœur et 256Mo de mémoire pendant 15 minutes. Sur Trillium, la tâche réserve le nœud entier avec toute sa mémoire.
...
Pour compliquer davantage, K, M, G, etc. sont interprétés par Slurm comme étant des préfixes binaires; ainsi --mem=125G équivaut à --mem=128000M. La quantité de mémoire que vous pouvez demander est indiquée dans le tableau Caractéristiques des nœuds pour Fir, Narval, Graham,  Narval
et Nibi.
...
Le nom du fichier peut contenir certains symboles de remplacement, par exemple l'ID de la tâche, le nom de la tâche ou l'ID du vecteur de tâches. Voyez la page sbatch pour la liste complète.
...
Les tâches intensives avec MPI peuvent utiliser plus d'un nœud. Il est aussi possible d'avoir des tâches hybrides qui sont à la fois exécutées en MPI et en fils multiples. Pour plus d'information sur les tâches distribuées en parallèle, consultez Contrôle de l'ordonnancement avec MPI.

=== Tâche GPU (avec processeur graphique) ===
Pour des informations et des exemples de comment demander les ressources GPU, voir Ordonnancement Slurm des tâches avec GPU.
...
Voir la page wiki Performance des tâches.
...
Pour les calculs nécessitant une durée plus longue que la limite de temps du système, l'application doit pouvoir gérer des points de contrôle
...
Voir Restrictions particulières à Trillium.
...
Une tâche dépendante soumise avec --dependency=afterok: attend que la tâche parent soit terminée avant de s'exécuter. Si la tâche parent s'arrête avant sa fin (c'est-à-dire qu'elle produit un code de sortie non nul), la tâche dépendante ne sera jamais exécutée et elle est automatiquement annulée. Pour plus d'information sur la dépendance, voir sbatch.
...
L'erreur suivante peut survenir si une condition n'est pas satisfaite :
 ...
 loaded as requested: "/"
    Try: "module spider /" to see how to load the module(s).
...
* Consultez Politique d'ordonnancement des tâches pour des renseignements sur la politique de priorisation des tâches et connaître les éléments pouvant influer sur l'ordonnancement de vos tâches.
* Si des tâches dans votre groupe de recherche sont en concurrence entre elles, consultez Gestion des comptes Slurm.

== Pour plus d'information ==
* SchedMD : documentation Slurm et tutoriels
** options pour la commande sbatch
* correspondance de commandes et directives Slurm avec PBS/Torque, LSF, SGE et LoadLeveler
* CÉCI, Belgique : tutoriel Slurm
* Bright Computing : tutoriel concis Slurm sous Unix
