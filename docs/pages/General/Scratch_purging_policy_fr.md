...
url: "https://docs.alliancecan.ca/wiki/Scratch_purging_policy/fr"
category: "General"
last_modified: "2025-07-27T17:34:24Z"
page_id: 6064
display_title: "Répertoire /scratch ː Purge automatique"
language: "fr"
---
...
Remarque : le système de fichiers /scratch de Graham et Nibi n'expire pas comme tel, mais utilise plutôt un quota pour forcer l'utilisation temporaire.
* Si vous utilisez moins de 1To, vous n'avez rien à faire (pas d'expiration, etc.).
...
* À l'expiration de ce délai, la procédure de dépassement de quota s'applique et interdit toute nouvelle allocation. (Ceci fait que vos fichiers sont toujours accessibles à ce stade, mais l'ajout ou l'augmentation de la taille des fichiers échouera et générera une erreur.)
* Pour résoudre ce problème, votre utilisation doit être modifiée et être sous la limite souple (ce qui remet le compteur à zéro).
* Il existe également un quota fixe de 20 To.

L'avantage de ce mécanisme basé sur les quotas est de vous laisser le plein contrôle et d'empêcher que certains fichiers particuliers d'une collection disparaissent en raison d'un âge quelque peu différent.
