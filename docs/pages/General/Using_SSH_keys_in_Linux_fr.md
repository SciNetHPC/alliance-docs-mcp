...
url: "https://docs.alliancecan.ca/wiki/Using_SSH_keys_in_Linux/fr"
category: "General"
last_modified: "2023-02-05T16:46:15Z"
page_id: 658
display_title: "Utiliser des clés SSH sous Linux"
language: "fr"
---
...
À l'invite, entrez une phrase de passe. Si vous avez déjà sauvegardé une paire de clés avec leurs noms par défaut, vous devriez donner un nom différent au fichier pour éviter d'écraser les clés existantes. Voyez plus d'information sur les  meilleures pratiques.
...
Copiez le contenu de la clé publique (dans notre exemple, id_rsa.pub) et téléversez-la dans la CCDB tel que décrit à l'étape 3 de cette procédure.
...
En supposant que vous ayez installé votre paire de clés dans l'un des emplacements habituels, la commande  ssh-add devrait la trouver mais, si nécessaire, vous pouvez explicitement ajouter le chemin complet de la clé privée en tant qu'argument à ssh-add. L'option ssh-add -l affiche les clés privées qui ont été ajoutées l'agent ssh.

Le programme ssh-agent permet de négocier automatiquement l'échange de clés entre votre ordinateur personnel et la grappe mais, si vous devez utiliser votre clé privée sur la grappe elle-même, par exemple lors de l'interaction avec un référentiel GitHub distant, vous devrez activer le transfert d’agent. Pour activer cette fonction sur une grappe  (Béluga dans l'exemple exemple), ,ajoutez les lignes suivantes à votre fichier $HOME/.ssh/config sur votre ordinateur local.

Notez que le fichier de configuration SSH ne doit jamais contenir  la ligne Host * $HOME/.ssh/configour le transfert de l'agent.
