...
url: "https://docs.alliancecan.ca/wiki/Using_node-local_storage/fr"
category: "General"
last_modified: "2025-08-31T14:11:30Z"
page_id: 14273
display_title: "Stockage local sur les nœuds de calcul"
language: "fr"
---

Quand l'ordonnanceur Slurm démarre une tâche, un répertoire temporaire est créé sur chacun des nœuds qui sont assignés à cette tâche.
...
Pour plus d'information, voir Tâches utilisant plusieurs nœuds et Espace disponible ci-dessous.
...
$SLURM_NNODES --ntasks-per-node1 cp file [files...] $SLURM_TMPDIR}}
...
$SLURM_NNODES --ntasks-per-node1 unzip archive.zip -d $SLURM_TMPDIR}}

=== Tarball ===
Pour extraire vers SLURM_TMPDIR :
$SLURM_NNODES --ntasks-per-node1 tar -xvf archive.tar.gz -C $SLURM_TMPDIR}}

= Espace disponible=

Dans le cas de Trillium, $SLURM_TMPDIR est implémenté comme RAMdisk; l'espace disponible est donc limité par la mémoire du  nœud, moins la capacité de RAM utilisée par votre application.
...
Si votre tâche réserve des nœuds entiers, il est raisonnable de penser qu'autant d'espace $SLURM_TMPDIR sera disponible sur chaque nœud.
...
À chacun des sites, certains nœuds ont plus d'espace disque local qu'indiqué dans le tableau; voyez la section Caractéristiques des nœuds pour chacune des grappes (Fir, Narval, Nibi, Rorqual).