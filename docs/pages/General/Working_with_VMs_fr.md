...
url: "https://docs.alliancecan.ca/wiki/Working_with_VMs/fr"
category: "General"
last_modified: "2023-03-06T16:46:02Z"
page_id: 21651
display_title: "Travailler avec les instances (machines virtuelles)"
language: "fr"
---
...
Il est possible de modifier la taille d'une instance en changeant son gabarit. Il y a toutefois certains points à considérer selon que le gabarit est p ou c (voir Gabarits d'instances). Le fait de supprimer une instance peut causer des problèmes car l'instance est en quelque sort supprimée puis recréée avec un nouveau gabarit; en cas de doute, demandez l'avis du soutien technique.

===Gabarits c===
Les gabarits c ont des disques éphémères additionnels dont la taille peut être modifiée en sélectionnant un autre gabarit c. La taille de ces disques éphémères ne peut pas être diminuée et donc les instances de gabarit c ne peuvent être modifiées que par des gabarits avec des disques de taille égale ou supérieure. Une fois la taille modifiée, vous ne verrez pas un disque éphémère plus grand dans votre instance, c'est-à-dire que la commande df -h ne montrera pas l'augmentation. Pour voir l'espace ajouté, il faut modifier la taille du système de fichiers (voir la commande resize2fs). La modification d'un système de fichiers peut toutefois prendre beaucoup de temps si les partitions sont larges; faites une copie du système de fichiers avant de procéder (voir  Sauvegarder une instance).
